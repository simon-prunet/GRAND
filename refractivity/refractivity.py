...
######### Approximations to complementary error function erfc
...
######### The following is based on the Fadeeva package from S. Johnson ########

def fadeeva_erfcx_ycent(y100):
	...
	71: seventyone(y100),
	72: seventytwo(y100),
	73: seventythree(y100),
	74: seventyfour(y100),
	75: seventyfive(y100),
	76: seventysix(y100),
	77: seventyseven(y100),
	78: seventyeight(y100),
	79: seventynine(y100),
	80: eighty(y100),
	81: eightyone(y100),
	82: eightytwo(y100),
	...
	return(switcher.get(iy100))

def fadeeva_erfcx(x):
	## Scales erfc function exp(x^2) erfc(x), defined here only for x>=0
	if (x<0):
		print('In fadeeva_erfcx: negative arguments not allowed')
		return
	if (x>50):
		# Continued fraction
		if (x>5e7):
			# Keep only first term to avoid underflow
			return 1./(np.sqrt(np.pi)*x)
		else:
			## 5 terms expansion
			return (((x*x) * (x*x+4.5) + 2) / (x * ((x*x) * (x*x+5) + 3.75))/np.sqrt(np.pi))
	else:
		return(fadeeva_erfcx_ycent(400./(4.+x)))

def fadeeva_erfc(x):
	## Normal version of erfc(x)
	if (isinstance(x,np.ndarray)):
		res = np.zeros_like(x)
		for i in range(x.size):
			if (x[i]<0):
				res[i]=2.0-np.exp(-x[i]**2)*fadeeva_erfcx(-x[i])
			else:
				res[i]=np.exp(-x[i]**2)*fadeeva_erfcx(x[i])
		return(res)
	else:
		if (x<0):
			return (2.0-np.exp(-x*x)*fadeeva_erfcx(-x))
		else:
			return np.exp(-x*x)*fadeeva_erfcx(x)
