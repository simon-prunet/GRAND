...
def CreateAiresZernikeQuadInp(zenith, azimuth, alpha, az_slope, cone_vertex=100000.0, cone_ang=2.0, nmax=20, mmax=4., az_B=0.0, zen_B=147.4, outputfile='TestInput.inp', outmode="a", RandomFraction=0, projection="Geometric"):
  #==============================================================================
  '''
  Derived from CreateAiresStarShapeInp, using Zernike quadrature points for the starshape.
  :param zenith: Zenith[deg, AIRES]
  :param azimuth: Azimuth[deg, AIRES]
  :param alpha: Slope of the ground
  :param az_slope: Azimuth of the ground slope
  :param cone_vertex: Distance from the core to the cone vertex [m] (typically distance to xmax)
  :param cone_ang: Cone semi-aperture angle [deg]
  :param nmax: Maximum degree of radial part of Zernike polynomials
  :param mmax: Maximum degree of azimuthal part of Zernike polynomials
  :param az_B: Azimuth of magnetic field, to get the vXB components. Usually 0 if azimuth 0 points north (Aires Magnetic coordinates)
  :param zen_B: Zenith of magnetic field. 147.43 deg in Lenghu (152.95 deg in Ullastay)
  :param outputfile: Path and name of output file (generally suffixed in .inp)
  :param outmode: a for append, w to overwrite
  :param RandomFraction: If you want to add a fraction of antennas from a uniformly random distribution
  :param projection: 2 projection modes are implemented: "geometric" will project the starshape parallel to the shower axis. This gives an ellipse centered in (0,0)
                                                         "conical" will project the starshape through a cone with given vertex. This gives an ellipse with a focus at (0,0)
  '''
  import zernike
  DISPLAY=0
  PRINT=1
  ...
  ## Angles: uses ZHAireS angle convention
  ...
  az_B = np.deg2rad(az_B)  # North = Magnetic North
  zen_B = np.deg2rad(zen_B) #Direction where we point to
  ...
  print vxvxB
  print np.dot(v,vxB)
  print np.dot(v,vxvxB)
  print np.dot(vxB,vxvxB)
  ...
  # Define normalized (unit disc) quadrature points
  X,Y,W = zernike.zernike_quad(nmax,mmax)
  nant = X.size # Beware, here this is the total number of antennas in the star shape
  nrandom = int(nant*float(RandomFraction))

  #### star shape pattern in xyz
  xyznorm = np.zeros([nant+nrandom,3]) # original starshape without vxB, vxB rotation, no radius scaling
  xyz1=np.zeros([nant+nrandom,3]) # original starshape
  xyz=np.zeros([nant+nrandom,3])  # projection
  xyz2=np.zeros([nant+nrandom,3]) # back trafo in vxvxB
  xyz3=np.zeros([nant+nrandom,3]) # conical projection of starshape on ground

  #position of xmax
  XmaxPosition=v*cone_vertex


  max_ang = np.deg2rad(cone_ang)  # Most distant antenans are max_ang from axis
  radius_scale = cone_vertex*np.tan(max_ang)

  # Original starshape in plane perpendicular to shower axis
  for i in np.arange(nant):
    xyznorm[i,:] = (X[i]*ex + Y[i]*ey)
    xyz1[i,:] = radius_scale * (X[i]*vxB + Y[i]*vxvxB)
  # Geometric projection  
  for i in np.arange(nant):
    b=-np.dot(n,xyz1[i])/ np.dot(n, a)
    xyz[i]= xyz1[i] +b*a +r0 # projected
  # Conical projection
  for i in np.arange(nant):
    u=(0.0-XmaxPosition[2])/(xyz1[i,2]-XmaxPosition[2])
    xyz3[i]=(1-u)*XmaxPosition+u*xyz1[i]

  ##set of random test points
  for i in np.arange(nrandom):

    randomd= np.sqrt(float(random.uniform(0, 1))) #we use the square root to sample the area uniformly.
    randomangle= 2*np.pi*float(random.uniform(0, 1))

    xyznorm[nant+i] = randomd * (np.cos(randomangle)*ex + np.sin(randomangle*ey))
    xyz0 = radius_scale * randomd*(np.cos(randomangle)*vxB+np.sin(randomangle)*vxvxB) # pattern in xyz     xyz0 # z*v=0, since z=0
    xyz1[nant+i]=xyz0 # original starshape produced

    # intersection of shower and mountain plane
    b=-np.dot(n,xyz1[nant+i])/ np.dot(n, a)
    xyz[nant+i]= xyz1[nant+i] +b*a +r0 # projected
    ...
    xyz3[nant+i]=(1-u)*XmaxPosition+u*xyz0
  ...
  if PRINT:
      ...
      filenorm = open(outputfile+'norm', outmode)

      file.write('\n####################################################################################\n')
      file.write('# Starshape Pattern created with CreateAiresZernikeQuadInp v1.3\n')
      file.write('# mountain slope: {0:.2f} deg\n'.format(np.rad2deg(alpha)))
      ...
      filenorm.write('\n####################################################################################\n')
      filenorm.write('# Normalized coordinates associated to quadrature\n')
      filenorm.write('# Refence antennas coordinates correspond to quadrature\n')
      filenorm.write('# nmax: {0:d}\n'.format(nmax))
      filenorm.write('# mmax: {0:d}\n'.format(mmax))
      filenorm.write('\n####################################################################################\n')
      ...
      for i in np.arange(nant):
        ...
        filenorm.write("{0:11.5f} {1:11.5f} {2:11.5f}\n".format(xyznorm[i,0],xyznorm[i,1],xyznorm[i,2]))
      ...
      for i in np.arange(nrandom):
        if(projection=="geometric" or projection=="Geometric"):
          file.write("AddAntenna CrossCheckA{0:d} {1:11.2f} {2:11.2f} {3:11.2f}\n".format(int(nant+i),xyz[nant+i,0],xyz[nant+i,1],xyz[nant+i,2]))
        elif(projection=="conical" or projection=="Conical"):
          file.write("AddAntenna CrossCheckA{0:d} {1:11.2f} {2:11.2f} {3:11.2f}\n".format(int(nant+i),xyz3[nant+i,0],xyz3[nant+i,1],xyz3[nant+i,2]))
        filenorm.write("{0:11.5f} {1:11.5f} {2:11.5f}\n".format(xyznorm[nant+i,0],xyznorm[nant+i,1],xyznorm[nant+i,2]))
      file.write('####################################################################################\n\n')
  ...
  if DISPLAY:
    ...
    dirvxB=np.zeros([200,3])
    dirvxvxB=np.zeros([200,3])

    for i in np.arange(0,200):
      shower[i]= (i-100)*100 *a +r0
      mount_u[i]= (i-100)*100 *umountain +r0
      mount_v[i]= (i-100)*100 *vmountain +r0
      dirvxB[i]= (i-100)*100 *vxB + r0
      dirvxvxB[i] = (i-100)*100 *vxvxB + r0
    fig1=plt.figure(1, figsize=(12, 10), dpi=120, facecolor='w', edgecolor='k')
    title="zen_G="+str(np.rad2deg(zen_rad)) + " az_G="+str(np.rad2deg(az_rad)) + " slope=" +str(np.rad2deg(alpha))
    ...
    ax.plot(dirvxB[:,0],dirvxB[:,1],dirvxB[:,2], c='cyan', label="vxB")
    ax.plot(dirvxvxB[:,0],dirvxvxB[:,1],dirvxvxB[:,2], c='orange', label="vxvxB")
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.set_zlabel('z')
    plt.legend(loc = 'best')
    ...
    ax2.scatter(xyz3[:,0],xyz3[:,1],label="conical projection")
